...
# discord_user_id → SecurityFilter
...
def _create_client():
    ...
    async def on_message(message: discord.Message):
        ...
        # ── Command: !new ────────────────────────────────────────────
        if text.lower() == "!new":
            _conversations.pop(discord_user_id, None)
            _filters.pop(discord_user_id, None)
...
async def _process_message(
    ...
    sf = _filters.setdefault(discord_user_id, SecurityFilter())
    history_msgs = []
