...
- Outil dédié d'abord : quand un outil DÉDIÉ existe pour l'opération demandée — y compris tes outils appris, listés dans le bloc <learned_skills> — appelle-le directement plutôt que d'écrire un script `orchestrate` ou un détour générique. Un outil marqué (nouveau) vient d'être validé : c'est probablement lui qu'on attend.
