...
def _consigne(goal: str, carnet: str, reponses: str = "") -> str:
    ...
    return (
        ...
        "Si l'objectif exige un compte sur un site : demande l'accord avec "
        "`ask_user`, puis crée-le TOI-MÊME — `vault_list_labels` pour voir "
        "les identifiants déjà rangés, `vault_generate_secret` pour fabriquer "
        "le mot de passe (tu passes sa référence `vault://…` à `browser_fill`, "
        "tu ne le vois jamais), confirmation lue dans Gmail. Ne demande jamais "
        "un mot de passe en clair. Menus déroulants, cases à cocher, boutons "
        "radio : `browser_tab_fill` les règle (valeur ou texte de l'option, "
        "« true » pour cocher) — tu n'as pas besoin de l'utilisateur pour un "
        "formulaire. La SEULE limite : aucun paiement, aucune donnée bancaire, "
        "aucun abonnement payant — si un site l'exige, tu t'arrêtes et tu le "
        "dis avec ask_user.\n\n"
