...
# browser_tab_screenshot, os_screenshot, image_*, qrcode_*, etc.) peuvent écrire.
# Toute tentative d'attachement depuis un autre chemin est refusée pour éviter
# qu'un LLM halluciné dise `local_path="/etc/passwd"` ou similaire.
#   /tmp/ely-attachments  → browser_screenshot, image_*, qrcode_*…
#   /tmp/ely-screenshots  → browser_tab_screenshot (extension Chrome) — ajouté
#       2026-06-16 : sans lui, une capture de l'extension ne pouvait pas être
#       envoyée par mail (seul drive_upload_local_file la couvrait).
...
async def gmail_send_with_local_attachment(
    ...
    Use this when the file was just generated by another tool — for example:
      - `browser_screenshot` / `browser_tab_screenshot` return a `local_path`
        you can pass here
