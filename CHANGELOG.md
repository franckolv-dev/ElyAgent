...
> **Ely devient autonome, se souvient mieux et n'abandonne plus.** Neuf jours, vingt-huit PR. La mission ne se conclut plus sur la parole du modèle : un passage est toujours jugé, une limite annoncée n'est pas une demande satisfaite, un outil qui annonce son échec en texte est un échec. Elle peut poser une question et attendre, créer un compte sous accord, remplir un formulaire jusqu'au bout. Un événement (mail, agenda, document) peut la déclencher ; des critères de réussite bornent sa conclusion ; une écriture incertaine attend une vérification humaine plutôt qu'un rejeu. La mémoire est choisie à chaque demande, et la voix peut venir d'un service local à voix clonée. Les essais réels du 10 septembre sont nettement meilleurs que les précédents.

### Added
- **Autonomie** (PR #402, page **Autonomie** : Automatismes, Suivi, Autorisations, Procédures ; migration `0038_autonomy`, six tables) : un automatisme déclenche une mission à l'arrivée d'un mail Gmail, d'une modification de l'agenda principal ou d'un document nouvellement indexé — filtres, simulation sans exécution, activation/pause, curseurs persistants, plafond quotidien, événement traité une seule fois par règle, y compris après redémarrage. La vérification tourne toutes les minutes sans modèle de langage. Rien n'est créé ni activé à la place de l'utilisateur.
- **Critères de réussite** (PR #402) sur mission et automatisme : accusé d'outil, texte présent dans le bilan, sources citées et retrouvées. Une mission ne se déclare pas terminée avec un critère manquant ; un passage correctif, puis décision humaine. **Autonomie → Suivi → Critères et preuves** montre les vérifications et les accusés.
- **Accusé durable avant chaque écriture de mission** (PR #402) : déduplication des appels strictement identiques dans une même exécution ; un redémarrage ne transforme pas un accusé absent en succès ; une « exécution incertaine » attend la vérification de l'utilisateur (« L'action a bien eu lieu » / « autoriser un essai ») et bloque la relance à zéro. Reconnexion Chrome/Système attendue pendant 24 h ; les reports de fournisseur existants sont conservés.
- **Autorisations par utilisateur** (PR #402) : par catégorie (consultation, modifications privées, actes engageants, Ely Desktop, effets non classés), « selon mes autorisations », « toujours me demander » ou « interdire » — l'interdiction l'emporte, puis la confirmation. Notifications réglables, décisions consignées dans **[Missions] Notifications**.
- **Suivi des procédures apprises** (PR #402) : attribution prudente (procédure fournie ET outils réellement appelés), résultats « sans erreur / échec / non vérifié », suspension après trois échecs consécutifs, procédures épinglées exemptées, réactivation manuelle.
- **La mémoire choisie à chaque demande** (PR #402, migration `0039_memory_context`) : union réelle FTS5 + vecteurs fusionnée par rang (RRF), mesurée à 119/120 contre 105/120 pour la recherche dense seule (`scripts/bench_memory_methods.py`, `docs/evaluations/`) ; budget de 1 400 tokens en conversation et 2 400 en mission, tokenizer `cl100k_base` préchargé dans l'image ; calcul ou salutation = aucun rappel. **Analyse → Mes mémoires** : corriger un souvenir (l'ancienne valeur reste historique), le réserver à une mission ou un compte, l'épingler, l'oublier ; **Utilisé pour cette réponse** montre ce qui a été préparé, d'où, pourquoi, et sa taille. `memory_recall(memory_type="error")` retrouve les échecs d'outils du seul utilisateur.
- **Une capacité de calcul créée pendant la mission** (PR #402, `sandbox_save_tool` / `sandbox_run_tool`, **204 outils**) : Ely fournit `run(arguments)`, une description et un à cinq exemples ; le programme est contrôlé et exécuté en sous-processus, un test en échec interdit l'enregistrement ; la version immuable devient utilisable, par son seul auteur. Calculs et transformations en mémoire uniquement : ni comptes Google, ni réseau, ni fichiers.
- **« Connexion GPT interrompue »** (PR #402) : une bannière quand l'abonnement ChatGPT (codex) refuse l'authentification, avec **Reconnecter GPT** vers la carte du modèle ; l'état repose sur les appels observés, aucun appel payant n'est lancé pour vérifier.
- **Une mission pose une question et attend** (`5d5325c`, PR #393) : `ask_user`, statut `waiting_user`, `POST /api/missions/{id}/answer`, écart non résolu = question plutôt qu'échec. ⚠️ La mission RDV avait pris 217 actions et 3,7 M tokens parce qu'elle n'avait pas le droit de demander.
- **Une mission crée un compte sous accord** (`b69d10b`, PR #394) : coffre AES-GCM/Argon2id avec son UI, `vault_generate_secret`, `vault_list_labels`, références `vault://` résolues par la passerelle et **remasquées** dans les retours (fuite `browser_fill` fermée), playbook « créer un compte pour l'utilisateur ».
- **Menus déroulants, cases à cocher et boutons radio** (`bf138ed`, PR #395) dans l'extension Chrome (v1.9.0) et le navigateur serveur : l'inscription va au bout sans l'utilisateur.
- **Un niveau de routage « Missions »** (`c576e7f`, PR #396), vide par défaut, plafond porté à 10 M de tokens, bouton **Relancer** qui garde le carnet.
- **Une tâche planifiée peut lancer une mission** (`8994a85`, PR #383, migration `0036`) : anti-doublon, bouton « Nouvelle tâche » ; l'aide du champ cadence nomme ses cinq champs avec un exemple par cas (`46448eb`, PR #384).
- **La voix d'Ely peut être locale et clonée** (`f97d9041`, PR #398) : service XTTS-v2 à part (`voice/xtts/`, MPS, phrase par phrase sous verrou), `TTS_PROVIDER=xtts`, repli edge-tts journalisé, `GET /tts/status`, lecture phrase par phrase côté chat et mode voix. Le texte lu ne quitte plus la machine. Licence CPML non commerciale, modèle non embarqué.
- **Un halo tourne autour de la zone de saisie tant qu'Ely travaille** (`b0b8fb5`, `35658f8`, `f4d0dea`, PR #399→#401) : liseré conique et ombre portée multicolore floutée, deux thèmes, `prefers-reduced-motion` respecté.
- **Guides indexables** (PR #402) : `docs/guide-interface-ely.md`, `docs/guide-autonomie-ely.md`, `docs/guide-memoire-ely.md`, deux fiches d'aide, un plan de tests en huit niveaux (`docs/plan-tests-ely-2026-09-10.md`), une étude mémoire, une note sur les modèles locaux.

### Changed
- **Un résultat doit être établi** (PR #402) : le juge exige une tentative, une alternative ou la preuve d'un accès manquant ; de nouveaux appels réussis et distincts comptent comme progrès ; un panel sans outils fournit une stratégie, l'agent reprend ses outils et vérifie ; un juge indisponible ou illisible **ne valide plus** ; le juge essaie les autres fournisseurs de sa chaîne, y compris sur une erreur d'authentification.
- **La passerelle reconnaît davantage d'erreurs** (PR #402 ; `34f6aa2`, PR #388) : réponses JSON négatives, erreurs annoncées en texte (« Erreur », « Échec »), suggestion des outils voisins sur un nom inconnu ; elle ne répète jamais une écriture incertaine.
- **La sélection locale** (PR #402) ajoute les actions demandées aux outils de lecture du domaine ; un outil découvert reste lié aux relances ; une affirmation « je n'ai pas d'outil » sans recherche provoque une consultation du catalogue dans le même tour. Le marqueur `capability_lookup_forced` est retiré.
- **Une mission choisit ses familles d'outils au premier passage** (`f994b98`, PR #382) : 45 outils au lieu de 227 ; `find_tool` élargit ; **un passage compacte ses anciens retours d'outils** avant chaque appel et le journal cesse de lister 230 outils (`238e7ef`, PR #397).
- **Une mission ne se conclut plus sur la parole du modèle** (`feb38df`, PR #389 ; `afecdff`, PR #381) : passage toujours jugé, jamais `completed` sur des écarts ouverts, le budget consommé n'est pas un avancement, le juge lit la fin du passage, un budget épuisé clôt avec son bilan. Une mission ne s'ausculte pas (`6b3c15a`, PR #378) ; un tour reste sur COMPLEX (`8954536`, PR #377) ; le formulaire décrit un objectif, plus de YAML (`31f3b58`, PR #376).
- **Interface** (PR #402) : thèmes clair et sombre persistants, plus aucune police chargée depuis Google Fonts ; trois entrées de conversation ; panneau de présence avec modèle, latence et tokens réels, sans scores synthétiques ; menu mobile masqué quand fermé ; le défilement ne ramène plus en bas quelqu'un qui relit. L'avatar filaire d'origine est conservé (un portrait texturé a été essayé puis retiré à la demande de Franck). Le lecteur vocal libère ses ressources, interrompt les requêtes anticipées, ignore les réponses périmées ; le mode conversation et le lecteur automatique ne parlent plus en double.
- **Relancer une mission repart vraiment à zéro** (`2e8f9c0`, PR #386) : carnet, sélection d'outils et plan de session effacés — `/restart` n'effaçait que le plan de l'ancien exécuteur.
- **README EN et FR** portent le discours local/cloud (`291f7f9`, PR #391). Image Docker : tokenizer préchargé au build, GLiNER épinglé (PR #402).

### Fixed
- **Incidents et correctifs de liaison** (PR #402, `binding_repair.py`) : la vérification lit la demande réellement exécutée et persistée, plus la consigne courante ; une liaison illisible est ignorée sans neutraliser les autres ; une proposition périmée est refusée ; l'annulation préserve une modification faite entre-temps et rouvre l'incident. **« Générer un outil »** dit que la fabrique est gelée au lieu de ne rien faire (`35658f8`, PR #400).
- **Ely avait appris à se bloquer** (`5703959`, PR #385) : ce qu'un profil exclut, rien ne le rebranche ; sept procédures apprises interdisaient tout appel Gmail/Drive/Sheets — douze archivées. ⚠️ Une consigne de prompt devient une loi générale via la fabrique de compétences.
- **Une relance courte ne part plus en local** (`9a78fd9`, PR #392), et une annulation affirmée sans outil est bloquée.
- **Un garde préalable en panne suspend l'action** (`f28d088`, PR #387) ; deux noms non définis que la CI ne voyait pas — F821 activé.
- **Les fichiers produits par `python_execute` survivent au sandbox** (`29bbd2e`, PR #390). **Une erreur serveur du fournisseur se reporte et se replie** (`78b8058`, PR #379). **L'extension attend le délai demandé** (`43e6911`, PR #375). L'arrêt d'une exécution interrompt son processus Python (PR #402).

### Ce qui n'a PAS été fait, et pourquoi
- **La parité avec Hermes ou OpenClaw n'est pas démontrée** : il faut vingt missions reproductibles, même modèle et même budget — le plan est dans `docs/plan-tests-ely-2026-09-10.md`.
- **Les événements sont interrogés, pas reçus** : pas de webhook Gmail ou Agenda.
- **La déduplication couvre un appel identique**, pas deux formulations d'un même effet ; un accusé d'envoi n'est pas une preuve de livraison.
- **L'architecture mémoire proposée dans l'étude** n'est pas activée au-delà de la sélection par demande.
...
### Fixed
- **La voie locale ignorait qu'elle avait un catalogue** (`bf4e81e`, PR #341) : sur « trouve-moi des sites comme Babelio », Ely a répondu qu'elle n'avait « pas accès à un outil permettant de rechercher des sites web », puis « je ne sais pas si un tel site existe ou non ». `web_search` existe, avec six fournisseurs en cascade derrière lui — et la seconde phrase est pire que la première : c'est une conclusion sur **le monde**, tirée par un modèle qui n'a rien cherché. Ce n'est pas une hallucination, c'est une conséquence : le 21/08 (#329) la voie locale a cessé de recevoir le catalogue, `find_tool` étant désigné comme le filet vers tout le reste. **Le filet a été tendu et personne n'a dit au modèle qu'il existait.** Le prompt parlait des outils « disponibles » ; de son point de vue aucun ne cherchait sur le web, donc déclarer l'incapacité était littéralement la réponse honnête. Il nommait même quatre outils que la voie locale ne lie plus (`calendar_list_events`, `gmail_list_emails`, `contacts_search`, `scheduler_list_tasks`) — le modèle les récitait comme étant les siens. ⚠️ **Connaître l'outil ne suffisait pas** : au tour suivant, Ely a nommé `find_tool` et `report_missing_capability`, décrit correctement les deux, et n'en a appelé aucun — elle a demandé la permission d'appeler un outil de **lecture**, puis servi sa plomberie en guise de réponse. Sur trois tours, elle a proposé une action au lieu de la faire trois fois. Le prompt lui retire désormais l'option de raconter. ⚠️ Moitié mécanique indissociable : les outils rendus par `find_tool` sont enfin **liés au tour SLM suivant**, comme sur la voie cloud. Le filet ne fonctionnait que par un accident de scoring — le retour de `find_tool` dépasse 80 caractères, ce qui repassait la barre de complexité et renvoyait le tour au cloud. Raccourcir ce message l'aurait cassé sans que rien ne rougisse. Et le cas inverse est celui qui compte : quand `find_tool` ne trouve rien, sa réponse est courte et le tour **reste local** — précisément là où le gap doit être consigné. ⚠️ Même classe de défaut qu'en #319 : *un modèle qui ne voit pas l'outillage ne doit pas témoigner de son absence.* La règle existait ; elle n'avait jamais été portée sur une voie qui n'était pas encore empruntable.
- **Changer le modèle d'une instance n'avait aucun effet** (`63e36c6`, PR #342) : tier A basculé de `nemotron-3-nano-4b` vers `gemma-4-E4B`, enregistré, réaffiché correctement dans Réglages → Routage — et LM Studio chargeait toujours nemotron à chaque question. Le nom n'était **ni en base, ni dans le `.env`, ni à l'écran** : il ne vivait plus que dans un objet Python, le client construit au démarrage et gardé dans la fermeture de `create_agent_node`. Un tier peut désigner une instance nommée par son UUID, et c'est l'instance qui porte le nom du modèle ; l'éditer change donc quel modèle sert un tier, mais `register_instance_cache` mettait à jour le cache **sans incrémenter le compteur d'invalidation** — le seul signal dont dispose `nodes.py`. ⚠️ **C'est la classe de défaut la plus chère du dépôt** : les trois surfaces d'observation sont justes — la base a la bonne valeur, l'API la rend, l'écran l'affiche. Seul le comportement ment, et il n'existe aucun endroit où regarder pour s'en apercevoir. ⚠️ **Troisième occurrence, troisième porte** : #272 (fenêtre de contexte et tarifs), #336 (modèle du tier A côté SLM), celle-ci (modèle d'une instance). Un pin structurel vérifie désormais que *toute* fonction écrivant dans `_instance_cache` incrémente le compteur — la quatrième porte rougira avant d'exister. Effet de bord bienvenu : le chargement paresseux de `get_llm_for_tier` prend enfin effet au tour suivant au lieu d'attendre un redémarrage.

### Docs
- `docs/architecture.md` gagne **« La voie locale, et son unique porte »** : que le tier A ne reçoit pas le catalogue, et que `find_tool` est par conséquent la seule route entre le modèle local et la météo, l'agenda, les mails ou la recherche web. Le fait qui a rendu la #341 possible n'était écrit nulle part.

---

## [2.4.0] — 2026-08-22

> Lot du 2 au 22 août 2026 (PRs #318→#337). **Trois semaines à réparer ce qui existait**, puis un chantier de roadmap livré. Fil conducteur, encore plus net qu'en 2.3.0 : un mécanisme qui **échoue en silence** coûte plus cher qu'un mécanisme absent. La voie rapide locale dépassait son délai depuis des semaines, un dossier surveillé affichait « scan en cours » depuis des mois, et des appels LLM facturés n'apparaissaient dans aucun chiffre. Aucun de ces trois-là ne plantait.

### Added
- **Automatisation web sans navigateur ouvert** (`db93a98`, PR #337) : `web_screenshot`, `web_to_pdf`, `web_extract`, `web_compare`. Une URL entre, un résultat sort — contexte Chromium créé, utilisé, fermé. Conçu pour les **tâches planifiées**, qui tournent sans personne devant et parfois pendant que l'utilisateur navigue : réutiliser sa session la lui déplacerait sous les yeux, et le résultat dépendrait de l'endroit où il l'a laissée. `web_compare` sert la veille — on lui donne le texte relevé la fois d'avant, il dit ce qui a changé. ⚠️ La ligne de roadmap qui portait ce chantier était **inexacte** : elle annonçait qu'il manquait Playwright, alors que `browser_skill` exposait déjà capture et extraction côté serveur. Le manque réel était l'**isolement**, pas l'outillage — le vérifier a évité d'écrire deux outils en double. ⚠️ Seuls `http://` et `https://` sont acceptés : `file://` donnerait accès au disque du conteneur par la porte d'un outil qui annonce « web ».
- **Indexation de dossier sans rien à lancer** (`aea8a7d`, PR #335) : `ELY_INDEX_PATH` monte un dossier dans le conteneur, **en lecture seule**, au **même chemin absolu des deux côtés** — `/Users/toi/Documents` existe alors tel quel côté serveur, donc aucune traduction de chemin et les citations RAG restent vraies. L'indexation cesse de dépendre du démon ELY Desktop. Celui-ci reste la voie des dossiers non montés : en ajouter un ne demande alors ni édition de compose ni redémarrage.
- **Démarrage automatique d'ELY Desktop sur macOS** (`aea8a7d`, PR #335) : LaunchAgent avec `RunAtLoad` et `KeepAlive`. ⚠️ **C'était le trou**, et il expliquait le reste : le bloc d'autostart de `install.sh` était sous `if [[ "$OS" == "Linux" ]]`. Windows avait sa clé de registre, Linux son entrée XDG, **macOS n'avait rien** — le script finissait par un `exec` au premier plan, donc fermer le terminal tuait le démon. macOS hérite au passage du meilleur des trois : `KeepAlive` relance après un plantage, ce que ni l'autostart XDG ni la clé Run ne savent faire.
- **Le catalogue complet est branché** (`f355c29`, PR #323) : ~145 outils au lieu du profil réduit, sauf sur les petites fenêtres de contexte.
- **La famille `pao` de recherche, le Sprint 2.5 de la mémoire, le thème sombre en ardoise** (`c79ac26`, PR #324).

### Fixed
- **La voie rapide locale se sabotait elle-même** (`711409b`, PR #329) : le tier A recevait `bind_tools(registry.all_tools)` — **~145 schémas d'outils**, plusieurs dizaines de milliers de tokens, à un modèle de 4 milliards de paramètres, pour répondre « bonjour ». Ce n'est pas de l'inférence, c'est du *prompt processing* : le modèle était chaud, le routage l'avait bien choisi, et l'appel dépassait 60 s. **Deux fois.** Le mécanisme annulait sa propre raison d'être. Un jeu dédié de deux outils (`find_tool` reste le filet) ramène la réponse à quelques secondes. ⚠️ Le profil `_DEFAULT_TOOLS` n'aurait pas suffi : 84 outils, un ordre de grandeur sous le problème.
- **Le repli local → cloud était muet** (`711409b`, PR #329) : la bascule ENTRE FOURNISSEURS émettait un toast depuis longtemps, celle du local vers le cloud non. Une journée passée à demander pourquoi « GPT-5.6-sol » répondait à « bonjour » alors que le modèle local était chargé et prêt — la réponse tenait en trois lignes de `docker compose logs`. ⚠️ Le signalement est **sans état**, contrairement à `try_activate` : un modèle local lent une fois ne doit pas être écarté pour toute la conversation.
- **Réparer une voie morte, c'est l'emprunter pour la première fois** (`d48f24b`, PR #330) : le premier tour qui a réellement emprunté la voie locale a planté — réponse affichée une demi-seconde, puis « une erreur interne s'est produite » l'écrasait. `_fb_state` était initialisé **dans** la branche cloud et relu sans condition au pied du nœud. Le défaut datait de #265 et était **inatteignable** tant que le SLM échouait systématiquement : trois semaines de suite verte sur du code jamais exécuté.
- **Un modèle optionnel injoignable coûtait le service** (`3429177`, PR #328) : `SLM_ENABLED=true` empêchait Ely de démarrer. Le warm-up était **attendu** dans le lifespan, brûlait 62 s de backoff, et **ne pouvait pas aboutir** — il interrogeait un serveur Ollama inexistant pour un nom de modèle qui n'était pas le bon. Le healthcheck du compose abandonnait avant. Un confort était devenu une dépendance dure du boot.
- **La voie locale nommait un réglage mort au lieu du modèle servi** (`199d22e`, PR #327) : l'étiquette **et la fenêtre de contexte** étaient calculées sur `SLM_MODEL`, que `get_slm()` ne lit pas. Conséquence la plus coûteuse : `fit_messages_to_context` cherchait la fenêtre d'un nom fantôme, retombait sur 8 192 tokens et **tronquait l'historique** d'un modèle qui en tenait bien plus.
- **Un appel LLM facturé qui n'apparaît nulle part** (`55fe931`, PR #325) : la cascade de **construction** (`get_llm_for_tier`) rendait un rang inférieur sans laisser la moindre trace — ni bascule enregistrée, ni toast, ni log. Mesuré sur le tier `complex` : **994 requêtes DeepSeek v4 Pro à ~42 500 tokens** pour **une seule** bascule enregistrée. La facture le voyait, Ely non.
- **Un scan « en cours » depuis des mois ne scannait rien** (`537c03e`, PR #334) : `scan_folder` posait `last_scan_status = "running"` puis sortait par `return` sur ses deux échecs les plus fréquents, **avant** le bloc de persistance. La ligne restait gelée. Et le cron horaire sautait les dossiers hors ligne **en silence** — il a tourné toutes les heures pendant des mois sans laisser une trace. ⚠️ La docstring du module **énumérait** les modes d'échec qu'il était censé enregistrer : un commentaire faux au bon endroit coûte plus cher que pas de commentaire.
- **Un incident sans sortie pollue la liste à vie** (`1940cea`, PR #332) : un incident dont la tâche planifiée a été supprimée ne pouvait plus être tranché — toute action rejouait la même erreur. Nouveau statut `obsolete`, posé par le service : « rejeté » aurait voulu dire « l'hypothèse est mauvaise », alors qu'elle est seulement devenue sans objet. Et `status: exists` cesse d'être présenté comme un échec — c'est la **confirmation** de l'hypothèse, le backend nommant l'outil qui couvre déjà la capacité.
- **Un tour local ne se comptait pas, et pas sous le bon nom** (`293ba61`, PR #331) : le panneau SESSION lisait `input_tokens` / `output_tokens` sur le websocket, que le backend **n'a jamais émis** — « TOKENS » valait « — » sur tous les tours depuis l'origine. Et LM Studio nommant ses modèles `nvidia/nemotron-3-nano-4b`, le tableau de bord attribuait les tours locaux à un « fournisseur » nommé nvidia. ⚠️ L'estimation de repli **annonce** qu'elle en est une (`~`) : une estimation muette se lit comme une mesure.
- **Une occurrence manquée se voit, un silence se mérite** (`3cf034a`, PR #326) : une tâche planifiée affichait une dernière exécution sans avoir rien livré. Le marqueur `[SILENT]` passe en égalité stricte et sous permission, et la conversation est **renommée** au lieu d'être supprimée.
- **Le thème sombre : le brouillard venait du texte** (`c8045a3`, PR #333) : les fonds étaient déjà ceux de la maquette ; c'est le texte qui avait été remonté au nom du contraste WCAG, resserrant les tiers entre eux et les rapprochant des fonds clairs. ⚠️ Un arbitrage défendable ligne à ligne peut détruire un rendu d'ensemble.
- Trois pannes silencieuses du 2 août (`e3a9fe9`/`fb3a272`/`0dc9ae0`, PR #318/#319/#320) : chaque fournisseur part avec **sa** clé et non celle du voisin ; le panneau ne se déclare plus témoin d'outils qu'il n'a pas ; une répétition n'évince plus ce qui a produit un résultat.
- **Une relance ne rejoue pas un acte déjà accompli** (`916b967`, PR #321) et **un seul chemin de livraison** pour les tâches planifiées (`c826d6f`, PR #322).

### Changed
- **Le cache SLM suit enfin le routage** (`5a46c3f`, PR #336) : changer le modèle du tier A dans Réglages → Routage ne reconstruisait pas le client — Ely servait l'ancien modèle jusqu'au redémarrage. ⚠️ Le défaut jumeau était **déjà corrigé** vingt lignes plus haut pour les tiers LLM, avec un commentaire qui le décrit mot pour mot (audit C-4, 06/05). Le site jumeau n'avait jamais été traité.
- **Les réglages SLM sont documentés** (`5a46c3f`, PR #336) : `docker-compose.yml` passait les quatre variables depuis toujours, `.env.example` n'en mentionnait aucune. ⚠️ Le plus trompeur est `SLM_MODEL`, **qui ne choisit pas le modèle** — le tier A le fait. Ce malentendu a coûté plusieurs jours.

### Docs
- README, guide utilisateur et architecture décrivent les **trois** familles de navigation, et le compte d'outils passe à **200**. La régression a été attrapée par `test_docs_match_the_code.py`, **deux fois** — le second document n'apparaissant qu'après correction du premier.
- La ligne « Automatisation web sans navigateur » sort de la roadmap, avec la raison pour laquelle son diagnostic était faux : **une entrée de roadmap est une hypothèse sur l'état du code, pas un constat**.
