...
- **Elle se crée ses propres outils quand une capacité manque — validation humaine obligatoire.** Demandez *« sais-tu convertir X ? »* ou butez sur un vrai manque en pleine tâche : ELY fouille son catalogue complet, consigne la capacité manquante, **génère un outil Python candidat dans la foulée** (12 secondes en prod pour un outil de distance de texte), le passe au pipeline de validation 7 étages, et vous notifie sur le téléphone. **Aucun code exécutable ne s'active sans votre promotion explicite** — c'est la ligne de sécurité structurelle. Une fois promu, l'outil est injecté avec une **fenêtre de grâce** (un outil fraîchement validé n'est jamais évincé du prompt par les anciens) et choisi par pertinence avec votre demande. La boucle a été démontrée en réel de bout en bout : elle crée, l'humain valide, elle utilise.
- **Elle mesure si l'apprentissage a vraiment marché.** Un skill promu n'est pas présumé bon : ELY peut **rejouer l'échec d'origine en mode shadow** (les résultats d'outils enregistrés sont re-servis — rien de réel n'est jamais ré-exécuté) avec et sans le skill et comparer les verdicts ; une passe hebdomadaire vérifie si le pattern d'échec **est revenu après la promotion**. Vos 👎 sur une réponse nourrissent la même boucle comme signal d'échec de plein droit.
- **Elle apprend des playbooks réutilisables de ses propres erreurs — toute seule.** Quand une tâche échoue (action refusée, blocage d'hallucination, mission mal notée, pouce baissé), ELY transforme l'échec en un court **playbook** Markdown — *« quand tu vois X, fais Y, jamais Z »* — le fait noter par un modèle-juge séparé, et **crée + active les bons d'elle-même** (du texte peut s'auto-activer ; du code exécutable jamais — voir ci-dessus). La bibliothèque démarre avec des playbooks de base, et vous pouvez **importer des playbooks communautaires** (`SKILL.md`) directement depuis une URL dans la file de revue.
- **Elle remarque vos routines et PROPOSE — sans jamais agir seule.** Un détecteur hebdomadaire 100 % heuristique (aucun LLM) repère les demandes qui reviennent à cadence régulière et propose d'en faire une tâche planifiée : notification push + formulaire de création pré-rempli. **C'est vous qui créez la tâche — ou refusez une fois, et la suggestion ne revient jamais.**
...
- **Missions structurées qui demandent au lieu de deviner** *(v1.17)*. Décrivez un workflow multi-étapes en YAML simple — `steps`, `foreach` sur les résultats d'un step précédent, et des **handlers d'edge-case** : `on_ambiguous: ask_user("…")`, `on_not_found: skip_with_note("…")`. Quand ELY hésite, elle **vous ping** (web, push, Telegram), vous répondez, elle reprend — pendant que les autres items continuent. Un viewer liste vivant montre chaque step et item (✓ ⏳ ⏸ ⊝) avec réponse inline. Pour les exécutions sans surveillance, un **mandat d'autonomie** (désormais un formulaire dans la modal mission — familles d'outils, budgets, comportement en cas imprévu) laisse les actions couvertes tourner sans confirmation à chaque geste — **validé une fois par vous, jamais auto-activé**, l'arrêt d'urgence le révoque, et le noyau protégé (ssh, vault, admin…) reste non mandatable.
...
Les signaux d'échec (refus HITL, blocages d'hallucination, critiques de mission, « capacité manquante », **vos 👎 sur une réponse**) alimentent une boucle d'apprentissage à deux produits distincts — des **playbooks Markdown** lisibles et des **outils Python** exécutables — chacun avec la ligne de sécurité qu'il mérite :

- **Capture autonome de playbooks.** Une passe de fond transforme les échecs récents en playbook — *« quand X, fais Y, jamais Z »* — un juge externe le note, et les bons sont **créés et activés automatiquement** (du texte peut s'auto-activer). La bibliothèque est **amorcée**, et vous pouvez **importer** des `SKILL.md` communautaires depuis une URL dans la file de revue admin (le contenu externe n'est jamais auto-activé).
- **Génération d'outils sur capacité manquante — sous validation humaine.** Une question méta (*« sais-tu convertir… ? »*) ou un vrai manque déclenche toute la chaîne d'elle-même : gap consigné dans le panneau **Capacités manquantes** → outil Python candidat généré (modèle tier S, allow-list AST → ruff → mypy → smoke sandboxé → enregistrement) → **notification push** → votre clic de promotion. **Le code exécutable ne s'active jamais tout seul.** Les outils promus reçoivent une **fenêtre de grâce** d'injection + un tri par pertinence pour être réellement utilisés, et le gap d'origine se referme automatiquement.
- **Mesuré, pas présumé.** `bench --case N` rejoue un échec enregistré en **mode shadow strict** (résultats d'outils capturés re-servis ; rien de réel n'est ré-exécuté) avec et sans le skill appris ; le curateur hebdomadaire suit si chaque pattern d'échec **est revenu après la promotion**.
...
Tout reste auditable et réversible — les playbooks sont de la prose que vous pouvez lire, modifier, archiver ; les outils générés sont des diffs que vous promouvez. Un banc de régression 50 scénarios + CI nocturne garde la boucle honnête.

> *Toujours gaté, désactivé par défaut :* la variante réseau **« io »** des outils (proxy egress filtrant, domaines déclarés, secrets injectés du Vault) et les outils touchant des fichiers — la frontière suivante, volontairement derrière des flags tant que le contrat fichier entrée/sortie n'est pas posé.
...
**Livré** *(juillet 2026)*
- **La boucle d'auto-développement, démontrée de bout en bout** — capacité manquante → outil candidat généré en quelques secondes → notification push → **validation humaine** → bindé, injecté avec fenêtre de grâce, utilisé. Les deux chemins prouvés en réel le même soir : l'heureux (outil de distance de texte, 12 s) et l'honnête (PDF→DOCX refusé par la validation, gap gardé visible) ([#217](https://github.com/franckolv-dev/ElyAgent/pull/217)–[#223](https://github.com/franckolv-dev/ElyAgent/pull/223))
- **Apprentissage mesuré** — replay shadow A/B des échecs enregistrés (rien de réel n'est ré-exécuté) + métrique de récurrence post-promotion ; les 👎 rejoignent le funnel d'échecs ([#225](https://github.com/franckolv-dev/ElyAgent/pull/225)–[#227](https://github.com/franckolv-dev/ElyAgent/pull/227))
- **Anticipation V1, en mode suggestion** — un détecteur heuristique hebdomadaire propose de transformer les demandes récurrentes en tâches planifiées (formulaire pré-rempli ; un refus est définitif) ([#228](https://github.com/franckolv-dev/ElyAgent/pull/228)–[#230](https://github.com/franckolv-dev/ElyAgent/pull/230))
- **Vérificateur de sortie sur les 8 surfaces + échéances murales + fallback honnête** — le garde anti-« c'est fait » couvre web, canaux, scheduler, voix et serveur MCP ; chaque appel LLM a une échéance dure qui déclenche la rotation de chaîne ; l'agent de secours hérite des résultats d'outils déjà acquis au lieu de déclarer « pas d'accès » ([#199](https://github.com/franckolv-dev/ElyAgent/pull/199), [#203](https://github.com/franckolv-dev/ElyAgent/pull/203), [#211](https://github.com/franckolv-dev/ElyAgent/pull/211)–[#215](https://github.com/franckolv-dev/ElyAgent/pull/215))
- **Formulaire de mandat d'autonomie** — la spec v2 de mission (familles d'outils, budgets, politique en cas imprévu) se génère depuis un formulaire dans la modal ; l'arrêt d'urgence révoque désormais le mandat ([#233](https://github.com/franckolv-dev/ElyAgent/pull/233), [#236](https://github.com/franckolv-dev/ElyAgent/pull/236))
